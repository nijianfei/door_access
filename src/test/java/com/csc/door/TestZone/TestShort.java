...
public class TestShort {
    ...
    public static void main(String[] args) {
        ...
        long controllerSN = 233131511;
        String controllerIP = "172.16.16.1";
        String watchServerIP = "172.16.121.1";
...
//代码  类型   英文描述  中文描述
    ...
    public static int testBasicFunction(String controllerIP, long controllerSN)  //基本功能测试
    {
        ...
        if (recvBuff != null) {
            ...
            controllerTime = String.format("20%02X-%02X-%02X %02X:%02X:%02X", WgUdpCommShort.getIntByByte(recvBuff[51]), WgUdpCommShort.getIntByByte(recvBuff[52]), WgUdpCommShort.getIntByByte(recvBuff[53]), WgUdpCommShort.getIntByByte(recvBuff[37]), WgUdpCommShort.getIntByByte(recvBuff[38]), WgUdpCommShort.getIntByByte(recvBuff[39]));
        } else {
            log("1.4 查询控制器状态 失败...");
            return 0;
        }
        ...
        if (recvBuff != null) {
            String controllerTime; //控制器当前时间
            controllerTime = String.format("%02X%02X-%02X-%02X %02X:%02X:%02X", WgUdpCommShort.getIntByByte(recvBuff[8]), WgUdpCommShort.getIntByByte(recvBuff[9]), WgUdpCommShort.getIntByByte(recvBuff[10]), WgUdpCommShort.getIntByByte(recvBuff[11]), WgUdpCommShort.getIntByByte(recvBuff[12]), WgUdpCommShort.getIntByByte(recvBuff[13]), WgUdpCommShort.getIntByByte(recvBuff[14]));
        ...
        while (true) {
            if (!queue.isEmpty()) {
                ...
                if (recvBuff[1] == 0x20) {
                    ...
                    String hex = bytesToHex(recvBuff);
                    log(String.format("接收到来自控制器hex = %s 的数据包..", hex));
                    ...
                    long qrDataLen = WgUdpCommShort.getLongByByte(recvBuff, 8, 4);
                    dealQRData(recvBuff);
                }
            } else {
                long times = 100;
...
//	  return 0;
    ...
    private static void dealQRData(byte[] recv) {
        ...
        if (qrDataLen >= 1) {
            ...
            log(String.format("流水号=%d 二维码字符串:\n        %s",
                    cmdSequenceId,
                    qrString));

//            // 远程开门处理
//            long recordCardNO = cmdSequenceId;
//            long sn = WgUdpCommShort.getLongByByte(recv, 4, 4);
//
//            byte[] buff = new byte[WGPacketSize];
//            java.util.Arrays.fill(buff, (byte) 0);
//
//            // 流水号递增逻辑
//            if (++sequenceId4RemoteOpen >= 0x7FFFFFFF || sequenceId4RemoteOpen < 0x40000001) {
//                sequenceId4RemoteOpen = 0x40000001;
//            }
//
//            // 构建数据包
//            buff[0] = 0x17; // Type
//            buff[1] = 0x40; // functionID
//
//            // 复制SN到缓冲区
//            ByteBuffer.wrap(buff, 4, 4)
//                    .order(ByteOrder.LITTLE_ENDIAN)
//                    .putInt((int) sn);
//
//            // 设置门号
//            buff[8] = (byte) (recordDoorNO & 0xFF);
//
//            // 进出状态取反
//            buff[28] = (byte) (recordInOrOut == 1 ? 0 : 1);
//
//            // 写入卡号（模拟）
//            ByteBuffer.wrap(buff, 20, 8)
//                    .order(ByteOrder.LITTLE_ENDIAN)
//                    .putInt((int) recordCardNO)
//                    .putInt((int) (recordCardNO >> 32));
//
//            buff[32] = 0x5A; // 控制标志
//
//            // 写入远程开门流水号
//            ByteBuffer.wrap(buff, 40, 4)
//                    .order(ByteOrder.LITTLE_ENDIAN)
//                    .putInt(sequenceId4RemoteOpen);
//
//            // 发送数据
//            int ret = server.UDP_OnlySend(buff);
//            String status = ret > 0 ? "成功" : "失败";
//            String direction = recordInOrOut == 1 ? "进门" : "出门";
//
//            log(String.format("\n    ====>%s 发出远程开门指令 控制器SN=%d, 门号=%d, %s 模拟卡号=%d\n",
//                    status, sn, recordDoorNO, direction, recordCardNO));
        }
    }
